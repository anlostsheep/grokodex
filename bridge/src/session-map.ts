...
import type { PermissionAudit, SessionMeta, SessionResumeReason } from "./types.js";
...
function emptyMeta(
  partial: Partial<SessionMeta> & Pick<SessionMeta, "reason">,
): SessionMeta {
  return {
    resumed: false,
    host_thread_id: null,
...
export function normalizeHostThreadId(
...
function extractDenyAllowTokens(cliArgs: string[]): string[] {
  const out: string[] = [];
  for (let i = 0; i < cliArgs.length; i++) {
    const a = cliArgs[i];
    if (
      (a === "--deny" || a === "--allow" || a === "--disallowed-tools" || a === "--tools") &&
      i + 1 < cliArgs.length
    ) {
 ...
 */
export function buildPermissionFingerprint(input: FingerprintInput): string {
...
export function createSessionMap(opts: { maxEntries?: number } = {}): SessionMapStore {
  const maxEntries = opts.maxEntries ?? 200;
  const entries: MapEntry[] = [];

  function key(h: string, f: string): string {
    return `${h}\0${f}`;
  }
...
let defaultMap: SessionMapStore | null = null;

export function getDefaultSessionMap(): SessionMapStore {
  if (!defaultMap) defaultMap = createSessionMap();
  return defaultMap;
}

/** Test helper: reset singleton. */
export function resetDefaultSessionMapForTests(): void {
  defaultMap = null;
}

export function resolveSessionPlan(input: ResolveSessionPlanInput): SessionPlan {
  const host = input.hostThreadId;
  const fp = input.fingerprint;
  const base = (reason: SessionResumeReason, extra: Partial<SessionMeta> = {}): SessionMeta =>
    emptyMeta({
      reason,
      host_thread_id: host,
      fingerprint: fp,
...
export function applyResumeCliFlags(
  args: string[],
  resumeSid: string | null,
): string[] {
...
export function markSessionMapUpdated(meta: SessionMeta, updated: boolean): SessionMeta {
  return { ...meta, map_updated: updated };
}

export function attachGrokSessionId(meta: SessionMeta, sid: string | undefined): SessionMeta {
  return {
    ...meta,
