 ...
 */
export function extractXSearchResults(
  text: string,
  limit: number,
): XSearchResultItem[] | null {
...
function tryParseJsonCandidate(s: string, out: unknown[]): void {
  if (!s) return;
  try {
...
function extractArraySubstrings(text: string, out: unknown[]): void {
  let i = 0;
  while (i < text.length) {
    if (text[i] !== "[") {
      i++;
...
function normalizeResultsArray(
  raw: unknown,
  limit: number,
): XSearchResultItem[] | null {
...
function normalizeMode(raw: string | undefined): XSearchMode {
  return raw === "keyword" ? "keyword" : "semantic";
}

function normalizeUsernames(raw: string[] | undefined): string[] | undefined {
 ...
 */
export async function handleGrokXSearch(
  args: GrokXSearchArgs,
  deps: GrokXSearchDeps,
  ...
  try {
    ...
    const resolvePermXSearch =
      deps.resolvePermXSearch ?? resolvePermissionForXSearch;
    ...
    const results =
      extractXSearchResults(text ?? "", limit) ??
      extractXSearchResults(result.stdout, limit);

    const notes: string[] = [...perm.audit.notes];
    if (!results) {
      notes.push(
